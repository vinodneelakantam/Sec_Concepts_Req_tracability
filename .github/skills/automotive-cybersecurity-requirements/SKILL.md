...
10 topic documents. The goal is **reality-grounded** requirements engineering for learning/interview
...
`SECOC` suffix) once it became clear the confidentiality/off-board/key-management content left
behind in the split-off COM doc was not substantial enough to justify two files. It now covers the
full secure communication stack: AUTOSAR SecOC per-PDU authenticity/freshness, confidentiality-
protected channels (SA2UL AES-GCM), off-board TLS/mTLS sessions, and shared key/session management.
