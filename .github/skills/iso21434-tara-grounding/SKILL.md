...
description: 'Ground new or existing TDA4VM ADAS ECU CSG/FSC/FSR/SYSR/TSC/TSR requirements in ISO 21434 Threat Analysis and Risk Assessment (TARA) vocabulary: assets, damage scenarios, threat scenarios/attack paths, impact rating (Safety/Financial/Operational/Privacy), attack feasibility rating, and risk treatment. Use when the user asks to justify, motivate, add rationale to, or run a TARA/threat analysis for a requirement, or asks "why do we need this requirement".'
---

# ISO 21434 TARA Grounding (TDA4VM ADAS ECU)

Adds the "why" layer underneath a CSG. This repo's requirement text (see
...
5. **Risk value** — combination of impact rating and attack feasibility; drives a **risk treatment**
   decision: reduce (add/strengthen a CSG), share/transfer, avoid (remove the feature), or retain
   (accept residual risk, usually only for Negligible impact).

## Procedure to ground a CSG

1. Identify the **asset** the CSG protects (name it precisely — "the SA2UL-derived DKEK", not
   "the key").
...
5. State the **risk treatment**: name the specific CSG/TSC/TSR ID(s) in this repo that reduce the
   risk, and briefly say how (e.g., "reduced via CSG-STO-2 + TSR-STO-1/TSR-STO-2, which bind secret
...
- **Risk treatment**: retained residual risk is low because CSG-STO-2 (device-bound confidentiality)
  and TSR-STO-1/TSR-STO-2 (KEK/DKEK derivation) mean the extracted ciphertext is not decryptable off
  the original device — no new CSG needed; existing ones already treat this threat scenario.
