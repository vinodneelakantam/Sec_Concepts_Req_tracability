...
- CSG-1: Verify integrity and authenticity of every boot-stage image before executing it.
- CSG-2: Detect tampering of running code/critical data after boot has already completed.
- CSG-3: Anchor verification in an immutable hardware root of trust, not a software-only check.
- CSG-4: Perform boot verification on every power-on/reset, independent of any prior flashing session.
- CSG-5: Gate post-reprogramming activation on the same trust chain used for ordinary power-on.
- CSG-6: Cover runtime integrity monitoring across power-on, post-reprogramming, and other operational scenarios.
...
- The eFuse row (`EFUSE`) is only ever a **read/compare** target, never a write target, from any
  PDX binary - it is the one component in this mapping that is not "copied to" (CSG-3, TSR-1).
...
- TSR-6: A boot-time authentication failure (TSR-1/TSR-2/TSR-3) at ordinary power-on shall halt with
  ...
  activation path (`TDA4VM_OTA_FOTA_SOTA_Requirements.md` CSG-OTA-4), never as an implicit behavior
...
- HWR-1: DMSC BootROM is the immutable root of trust that starts every boot (CSG-3, TSC-3, TSR-1)
...
- SWR-3: Post-flash activation reuses the identical DMSC BootROM -> SYSFW -> SBL -> Application
  authentication sequence; there is no separate/shortcut activation path (CSG-5, TSR-5)
...
- No bypass path: BootROM authentication of System Firmware runs on every power-on/reset with no
  disable option on HS-SE devices (CSG-1, CSG-4, CSG-5)
...
- Each stage's authentication failure halts release of the next core rather than falling through to
  a partially-initialized state - the chain fails closed, one stage at a time (CSG-1, TSC-1)
- Runtime detection continues after boot completion, independent of the boot-time chain (CSG-2, CSG-6)
